            ...
            for _object in root.findall('object'):
                ...
                for cat in categories:
                    if category == cat['name']:
                        cat_id = cat['id']
                xmin = float(_object.find('bndbox').find('xmin').text)
                ymin = float(_object.find('bndbox').find('ymin').text)
                xmax = float(_object.find('bndbox').find('xmax').text)
                ymax = float(_object.find('bndbox').find('ymax').text)
                w = xmax - xmin
                h = ymax - ymin
                if w < 0 or h < 0:
