...
class CocoDataset(BaseDataset):
    ...
    def get_train_data(self, idx):
        ...
        # img = cv2.imread(image_path, cv2.IMREAD_UNCHANGED)   # 直接读入单通道数据
        img = cv2.imread(image_path,  cv2.IMREAD_GRAYSCALE)        # 将3通道数据转成单通道数据
        ...
        meta['img'] = torch.from_numpy(np.expand_dims(meta['img'], axis=0))     # 扩展一个维度。
