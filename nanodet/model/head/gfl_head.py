...
class GFLHead(nn.Module):
    ...
    def show_result(self,image_name,img, dets, class_names, score_thres=0.3, show=True, save_path=None):
        raw_img = img.copy()
        result = overlay_bbox_cv(image_name,img, dets, class_names, score_thresh=score_thres)
        result = np.concatenate((raw_img, result), axis=1)
        if show:
