...
int resize_uniform(cv::Mat& src, cv::Mat& dst, cv::Size dst_size, object_rect& effect_area)
{
    ...
    if (tmp_w != dst_w) {
        ...
        for (int i = 0; i < dst_h; i++) {
            memcpy(dst.data + i * dst_w * n + index_w * n, tmp.data + n * tmp_w * n, tmp_w * n);
        }
        ...
        memcpy(dst.data + index_h * dst_w * n, tmp.data, tmp_w * tmp_h * n);
        effect_area.x = 0;
...
void draw_bboxes(const cv::Mat& bgr, const std::vector<BoxInfo>& bboxes, object_rect effect_roi, std::string save_path="None")
{
    ...
    for (size_t i = 0; i < bboxes.size(); i++)
    {
        ...
        cv::Scalar color = cv::Scalar(color_list[bbox.label][0], color_list[bbox.label][0], color_list[bbox.label][0]);
...
int main(int argc, char** argv)
{
    ...
    NanoDet detector = NanoDet("vit.mnn", 320, 320, 4, 0.45, 0.3);
