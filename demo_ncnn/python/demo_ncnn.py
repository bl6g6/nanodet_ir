...
class NanoDetABC(metaclass=ABCMeta):
    def __init__(self, input_shape=[320, 320], reg_max=7, strides=[8, 16, 32], prob_threshold=0.4,
        ...
        self.top_k = top_k
        self.img_mean = [27.40] # [103.53, 116.28, 123.675]
        self.img_std = [24.64] # [57.375, 57.12, 58.395]
    ...
    def detect_folder(self, img_fold, result_path):
        ...
        for img_path in tqdm(img_name_list):
            ...
            img = cv2.imread(img_path,  cv2.IMREAD_GRAYSCALE)        # 将3通道数据转成单通道数据
...
class NanoDetTorch(NanoDetABC):
    ...
    def infer_image(self, img_input):
        ...
        scores = [x.permute(0, 2, 3, 1).reshape((-1, len(self.class_names))).sigmoid().detach().numpy() for x in inference_results[0]]
...
class NanoDetNCNN(NanoDetABC):
    ...
    def infer_image(self, img_input):
        ...
        scores = [np.reshape(x, (-1, len(self.class_names))) for x in scores]
...
def main():
    ...
    parser.add_argument('--model_path', dest='model_path', type=str, default='../../mine_model/nanodet-m-persion-sim_0.25x.param')
    parser.add_argument('--model_bin', dest='model_bin', type=str, default='../../mine_model/nanodet-m-persion-sim_0.25x.bin')
    parser.add_argument('--cfg_path', dest='cfg_path', type=str, default='../../config/nanodet-m-persion.yml')
